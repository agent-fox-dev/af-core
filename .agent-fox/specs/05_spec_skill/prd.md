...
1. Create a skill prompt file (`speclib/skill/af-spec.md`) that instructs
   agent CLIs how to drive the spec authoring workflow using `af-spec` CLI
   commands.
...
3. Provide clear instructions for presenting assessment questions
   ...
   Question IDs.
4. Provide an `af-spec install-skill` CLI command that copies the skill file
...
- Implementing the Python API directly in the skill (the skill uses CLI
  commands only).
- Hub interaction (the skill only invokes `af-spec submit` when the user
  explicitly asks).
...
`af-spec` CLI. This avoids requiring agents to call the Python API directly,
...
includes examples of every relevant `af-spec` command.
...
| 04_af_spec_cli | 3 | 2 | Skill invokes af-spec CLI commands |
...
2. **CLI commands only.** The skill instructs the agent to use `af-spec`
...
4. **Shipped inside the package.** The skill file lives at
   `speclib/skill/af-spec.md` and is included in the Python package. An
   `af-spec install-skill` command copies it to the target location.
